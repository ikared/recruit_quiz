...
QuestionList CreatePrefecturesExam()
{
	...
	constexpr int quiCount = 5;
	QuestionList questions;
	questions.reserve(quiCount);
	...
	case 0:	//特徴から都道府県を答える
		for (int i = 0; i < quiCount; i++) {
			...
			const auto& features = data[correctIndex].features;
				vector<int> featuresIndices = CreateRandomIndices((int)features.size()); 
				s += features[featuresIndices[0]];
				for (int j = 1; j < features.size(); j++) {
					s += ",";
					s += features[featuresIndices[j]];
				}
