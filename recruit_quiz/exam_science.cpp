...
QuestionList CreatePhysicsExam()
{
	...
	{//等速直線運動
		...
		questions.push_back({
			...
			"kmで移動するのにかかる時間を分単位で求めよ", to_string(t) });
	...
	{//等加速度直線運動
		...
		questions.push_back({"秒速" + to_string(v0) + "mで移動していた車が、進行方向に大きさ" + to_string(a) +
			...
			"秒後の速度をm/s単位で求めよ", to_string(v0 * t + a * t * t / 2) });
		...
		questions.push_back({ "秒速" + to_string(v0) + "mで移動していた車がブレーキをかけたところ、" +
			to_string(v0 * t + a * t * t / 2) + "mで進んで停止した。\n" +
	...
	{//浮力
		...
		questions.push_back({
			"質量100gの物体に働く重力を1Nとする。\nある物体の重さをばねばかりではかると、" +
			to_string(x) + "gを示した。\nこの時物体を完全に水に入れたところ、ばねばかりは" +
			to_string(y) + "gを示した。\nこの時、物体に働く浮力はXニュートンである。\n" +
			"Xの値を小数点以下第2位で四捨五入して求めよ。", answer });
	...
	{//ばね
		...
		questions.push_back({ 
			"重力加速度を10m/s^2とする。\n長さ" + to_string(a) + "cmのばねの先端に" + to_string(m) +
			"gの重りをつけて、天井から吊り下げた。\nすると、ばねの長さが" +
		...
		questions.push_back({ 
			...
			"の2つのばねを直列につなぎ、" + to_string(m) + "gの重りをつけて、天井から吊り下げた。\n" + 
